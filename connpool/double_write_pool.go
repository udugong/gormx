...
func (tx *DoubleWriteTx) Commit() error {
	...
	case PatternSrcFirst:
		...
		if tx.dst != nil {
			err1 := tx.dst.Commit()
			if err1 != nil {
				tx.logger.Error("目标表提交事务失败", slog.Any("err", err1))
			}
		}
		return nil
	case PatternDstFirst:
		...
		if tx.src != nil {
			err1 := tx.src.Commit()
			if err1 != nil {
				tx.logger.Error("源表提交事务失败", slog.Any("err", err1))
			}
		}
		return nil
	case PatternDstOnly:
...
func (tx *DoubleWriteTx) Rollback() error {
	...
	case PatternSrcFirst:
		...
		if tx.dst != nil {
			...
			if err1 != nil {
				tx.logger.Error("目标表回滚事务失败", slog.Any("err", err1))
			}
		}
		return nil
	case PatternDstFirst:
		...
		if tx.src != nil {
			...
			if err1 != nil {
				tx.logger.Error("源表回滚事务失败", slog.Any("err", err1))
			}
		}
		return nil
	case PatternDstOnly:
